...
public:
  ...
  void drop_ball() {
    falling_sphere.body.setGravity(btVector3(0, -10, 0));
  }

  void set_ball_position(float horizontal_position, float vertical_position) {
    falling_sphere.body.setGravity(btVector3(0, 0, 0));
    falling_sphere.body.setLinearVelocity(btVector3(0, 0, 0));

    falling_sphere.body.setActivationState(3);

    // +0.2 so we can't place on ground...
    transform.setOrigin(btVector3(horizontal_position, vertical_position + 0.2, 0.0));
    falling_sphere.body.setCenterOfMassTransform(transform);
  }
...
public:
  ...
  void start() {
    ...
    float vertical_position = 1.5;
    world.set_ball_position(horizontal_position, vertical_position);
    ...
    while (running) {
      ...
      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, 0.0, 2.0)) {
        // Implementation needed
        world.set_ball_position(horizontal_position, vertical_position);
      }
      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &vertical_position, 0.0, 3.0)) {
        // Implementation needed
        world.set_ball_position(horizontal_position, vertical_position);
      }
      ImGui::End();
